...
public class 계산기 {
    ...
    함수정의 : 클래스{ } 안에서 작성
    함[상자] 수[숫자]란 ? 상자안에 들어있는 수 -> 상자안에 등러있는 코드
        - 왜? 미리 만들어진 코드 -> 미리 만들어서 반복적으로 사용하기 위해
        변수(값 저장) vs 함수(실행문 저장)

    자바의 함수 선언방법
        반환타입 함수명( 타입 매개변수명1 , 타입 매개명수명2 , 타입 매개변수명3 ~){
            다양한 실행문; |로직
            return 반환값
            }

        1. 반환타입 : return 문에 넣우준 값의 자료형/타입
...
int sumMethod2(){
    // Random().nextInt(); int형의 표현범위내 난수/랜덤
    return new Random().nextInt();
}

// 3. 매개변수가 없고 반환값도 없는함수
void  sumMethod3(){
